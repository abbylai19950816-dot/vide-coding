...
- 設定頁的「強制重建學員查詢」是例外維運工具，會主動重寫公開索引；只應在學生端查不到管理員已存在的方案、測試資料清理或索引疑似遺失時使用。
- 資料健康檢查若要確認學生端 lookup 是否存在，可在管理員端讀取預期的 `student_lookup/{hash}` 文件；這是維運讀取，不可搬到學生端，也不可在背景高頻自動執行。
...
- 強制重建公開索引會依學生/電話索引數產生批次寫入，但不會增加學生端讀取；這是低頻維運成本，不可變成每次登入或每次切換頁面的自動流程。
- `清除孤兒日誌成員` 只在管理員確認後重寫 `/data/course_logs` 一份文件；這是低頻維運寫入，不影響學生端讀取。
- `票券重算報告` 只使用管理員端已載入的 tickets/students/slots 本機資料產生報告；不新增學生端讀取，也不寫入 Firestore。
- `整理歷史票券紀錄` 只在管理員確認後重寫 `/data/tickets` 一份文件，新增 `reconcile_balance` 校準紀錄；這是低頻維運寫入，不影響學生端讀取。
