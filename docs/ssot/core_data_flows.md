...
- 學員自己在預約頁面買方案時，應先建立 `purchase_requests`，由管理員匯入成 `payments`，狀態預設 `unpaid`；管理員手動變更為 `paid` 後，才建立 `tickets` 並顯示學員可用堂數。
- 管理員在學員資料頁手動新增方案時，系統預設視為已確認收費，建立 `payments` 並標記 `paid`，同時建立一張對應 `tickets`。
- 若管理員手動新增方案時刻意改選待收款，學員不應立刻得到可預約堂數；之後按「標記為已收款」才可由同一筆 `paymentId` 建立一張票券。
...
未來只要新增、修改、移除系統功能，或強化登入、Firestore rules、tenant、多老師隔離、LINE Login 等資安架構，都必須先檢視本文件的核心資料流，並在完成前跑對應回歸測試。不能只測單一畫面是否可點擊，必須確認「收費、票券、預約、行事曆、出缺勤、課程日誌、學員查詢索引」是否仍同步。

每次修改核心資料流後，至少測：

- 學員購買方案，管理員收款後，學員可查到正確剩餘堂數。
- 管理員在學員資料頁手動新增方案時，預設已收款，且只建立一張對應票券。
- 管理員手動改為待收款方案時，學員不會提前取得可預約堂數。
...
任何功能變更或資安強化，都要回答以下問題：

- 這次變更會不會影響 `purchase_requests` 到 `payments` 的匯入？
- 這次變更會不會讓 `payments.status` 與 `tickets` 不一致？
- 同一 `paymentId` 是否仍只能產生一張 ticket？
- 預約、取消、改期、刪課是否仍走 `addBookingCascade()`、`cancelBookingCascade()`、`moveBookingCascade()`、`deleteSlotCascade()` 或等價 SSOT 流程？
- `slots[].bookings`、`students[].scheduledBookings`、`classes`、`course_logs` 是否仍以 `slotId` 正確串聯？
- 學員端是否仍只讀低成本公開資料與單一 lookup？
- 多老師或登入權限變更時，是否能保證不同老師與不同學生資料不可互看？
